...
def test_real_celery_worker_executes_through_redis() -> None:
    ...
    app.conf.update(task_ignore_result=False, result_backend=broker, worker_prefetch_multiplier=1)

    @app.task(name="mirror.live_smoke")
    def live_smoke(value: str) -> str:
        return f"ok:{value}"

    with start_worker(app, perform_ping_check=False, concurrency=1, pool="solo", loglevel="WARNING"):
