...
class Executor:
    ...
    async def _invoke(
        ...
        middleware_chain = self.middleware_chains.get(compiled.capability.name, self.middleware_chain)
        if middleware_chain is None:
            return await final(invocation)
        return cast(BaseModel, await middleware_chain.execute(invocation, final))
    ...
    def _can_run(run: ExecutionRun, step_id: str) -> bool:
        compiled = run.plan.get_step(step_id)
        return all(
            run.states[dependency] is StepState.SUCCEEDED
            for dependency in compiled.dependencies
        )
        ...
        def evaluate(node: ast.AST) -> Any:
            ...
            if isinstance(node, ast.BoolOp):
                ...
                if isinstance(node.op, ast.Or):
                    return any(values)
                raise ExecutionError(f"Unsupported boolean operator in {condition!r}")
