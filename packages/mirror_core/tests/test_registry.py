...
def test_register_capability():
    ...
    cap = CapabilityManifest(name="fetch", api_version="1.0", protocol="module:Protocol")
...
def test_register_provider():
    r = ExtensionRegistryManager()
    prov = ProviderManifest(name="httpx", capability="fetch", capability_api="~=1.0", factory="a:b")
...
def test_duplicate_capability():
    r = ExtensionRegistryManager()
    r.register_capability(CapabilityManifest(name="fetch", api_version="1.0", protocol="module:Protocol"))
...
def test_duplicate_provider():
    r = ExtensionRegistryManager()
    r.register_provider(ProviderManifest(name="httpx", capability="fetch", capability_api="~=1.0", factory="a:b"))
    with pytest.raises(RegistryError):
        r.register_provider(ProviderManifest(name="httpx", capability="fetch", capability_api="~=1.0", factory="a:b"))
